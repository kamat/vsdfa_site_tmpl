    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <!-- The above 3 meta tags *must* come first in the head; any other head content must come *after* these tags -->
    <title>[[*pagetitle]] - [[++site_name]]</title>
    <base href="[[!++site_url]]" />

    <!-- Bootstrap -->
    <!-- link href="assets/templates/kama/css/bootstrap.css" rel="stylesheet" -->

    <!-- OWL -->
    <link href="assets/templates/kama/css/owl.carousel.css" rel="stylesheet">
    <link href="assets/templates/kama/css/owl.theme.css" rel="stylesheet">
    <link href="assets/templates/kama/css/owl.transitions.css" rel="stylesheet">
    
    <!-- FancyBox -->
    <link href="assets/templates/kama/css/jquery.fancybox.css" rel="stylesheet">

    <!-- Kama + Bootstrap -->
    <link href="assets/templates/kama/css/style.css" rel="stylesheet">
    <!-- MAIN STYLE -- [[!mainStyleFile]] -->
    
    <!-- Let it snow! -->
    <link href="/snow/snow.css" rel="stylesheet">
    
    <!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
      <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
    <![endif]-->
    
    [[!testCookieAndSession]]